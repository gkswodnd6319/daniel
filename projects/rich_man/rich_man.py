...
from projects.rich_man.fx_data import (
...
def build_rich_man_tab():
    """Rich Man — left sidebar with Dashboard / Backtests / Paper Trade."""
    from projects.rich_man.fx_sim import build_backtest_tab
    from projects.rich_man.crypto_sim import build_crypto_backtest_tab
    from projects.rich_man.fx_paper import build_paper_trade_tab
    ...
    with ui.row().classes('w-full h-full gap-0'):
        ...
        with ui.column().classes('gap-0').style(
            ...
            for name, icon, label in [
                ...
                ('backtest_usd', 'attach_money', 'Backtest USD/KRW'),
                ('backtest_crypto', 'currency_bitcoin', 'Backtest Crypto'),
                ('paper', 'account_balance', 'Paper Trade'),
            ]:
        ...
        with ui.column().classes('flex-grow gap-0').style('overflow-y: auto;'):
            panels['dashboard'] = ui.column().classes('w-full')
            with panels['dashboard']:
                _build_dashboard()

            panels['backtest_usd'] = ui.column().classes('w-full')
            panels['backtest_usd'].set_visibility(False)
            with panels['backtest_usd']:
                build_backtest_tab()

            panels['backtest_crypto'] = ui.column().classes('w-full')
            panels['backtest_crypto'].set_visibility(False)
            with panels['backtest_crypto']:
                build_crypto_backtest_tab()

            panels['paper'] = ui.column().classes('w-full')
            panels['paper'].set_visibility(False)
            with panels['paper']:
                build_paper_trade_tab()
...
def _build_live_ticker(base):
    ...
    with ui.row().classes('w-full items-center justify-between'):
        with ui.column().classes('gap-0'):
            ...
            live_usd_label = ui.label('').classes('mono text-sm font-bold').style('color: #448aff;')
            with ui.row().classes('gap-4'):
                with ui.column().classes('gap-0'):
                    ui.label('USDT (Upbit)').classes('mono text-xs text-gray-600')
                    live_usdt_buy_label = ui.label('').classes('mono text-xs').style('color: #ff1744;')
                    live_usdt_sell_label = ui.label('').classes('mono text-xs').style('color: #00e676;')
                with ui.column().classes('gap-0'):
                    ui.label('USD (한투 est.)').classes('mono text-xs text-gray-600')
                    live_usd_buy_label = ui.label('').classes('mono text-xs').style('color: #ff1744;')
                    live_usd_sell_label = ui.label('').classes('mono text-xs').style('color: #00e676;')
    ...
    def update_live_chart():
        ...
        live_price_label.text = f'USDT \u20a9{latest:,.0f}'

        # Kimchi premium: compare USDT price vs official CRIX rate
        from projects.rich_man.fx_data import fetch_upbit_crix
        crix = fetch_upbit_crix()
        official = crix['USD'].get('basePrice', 0) if crix and 'USD' in crix else 0
        gap = latest - official if official else 0
        gap_pct = (gap / official * 100) if official else 0
        ...
        if official:
            ...
            live_usd_label.text = f'USD \u20a9{official:,.0f} (official)'
        else:
            live_usd_label.text = ''

        # Buy/sell rates with 90% 우대 spread
        eff_spread = 0.25 * (1 - 90 / 100) / 100

        # USDT rates (Upbit)
        usdt_buy = latest * (1 + eff_spread / 2)
        usdt_sell = latest * (1 - eff_spread / 2)
        live_usdt_buy_label.text = f'살 때: \u20a9{usdt_buy:,.2f}'
        live_usdt_sell_label.text = f'팔 때: \u20a9{usdt_sell:,.2f}'

        # USD rates (official - kimchi premium removed)
        if official:
            usd_buy = official * (1 + eff_spread / 2)
            usd_sell = official * (1 - eff_spread / 2)
            live_usd_buy_label.text = f'살 때: \u20a9{usd_buy:,.2f}'
            live_usd_sell_label.text = f'팔 때: \u20a9{usd_sell:,.2f}'
